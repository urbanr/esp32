...
- **Panel zahazuje zápisy, které začínají na lichém `x`.** Ověřeno: svislé čáry šířky 1 na x = 60, 180, 300 jsou vidět, na x = 121 a 241 ne. Kreslení písmen přímo přes `gfx` proto nic nenakreslí (každý bod glyfu je okno šířky 1) — vestavěné písmo ve velikosti 3 projde jen náhodou, protože kreslí obdélníky 3×3. Řešení: text vykreslit do `Arduino_Canvas` a poslat ho jedním blokem přes celou šířku (`draw16bitRGBBitmap(0, y, …, 450, h)`), tj. sudý začátek i sudá šířka. Aplikace s vlastním DMA to mají zadarmo, kreslí celé pruhy.
