...
- **Nejlepší výsledek na SD kartě** (`rat_hiscore.h`): SPI na vlastni sbernici HSPI (piny `SD_CS/SCLK/MOSI/MISO` z `../common/pin_config.h`), soubor `/rat-jumper/best.txt` s jedním číslem; čte se při startu, zapisuje při novém rekordu na konci hry. Bez karty se skóre pamatuje jen od zapnutí.
