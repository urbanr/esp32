...
// displeje); na displej jde po fyzickych pruzich 450 x STRIPE_H pres
...
static void crtPresent() {
  ...
  for (int y0 = 0; y0 < LCD_HEIGHT; y0 += STRIPE_H, stripeIdx++) {
    ...
    usWait += t1 - t0;
    usCompose += micros() - t1;
    queueCmd(0, 0x2A, LCD_X_OFF, LCD_X_OFF + LCD_WIDTH - 1, true);
    queueCmd(1, 0x2B, LCD_Y_OFF + y0, LCD_Y_OFF + y0 + STRIPE_H - 1, true);
